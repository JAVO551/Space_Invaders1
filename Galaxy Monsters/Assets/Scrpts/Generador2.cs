using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Generador2 : MonoBehaviour
{
    public float temporizador = 50;
    private float cuenta_atras;
     // Start is called before the first frame update
    void Start()
    {
        cuenta_atras = temporizador;
    }
    public GameObject[] enemigos2;
    private float tiempo_entre_generacion;
    public float tiempo_inicio;
    public float decremento;
    public float tiempo_minimo = 1;
    

    // Update is called once per frame
    void Update()
    {
        if (cuenta_atras <=0) {
            if (tiempo_entre_generacion <= 0) {
                int rand = Random.Range(0, enemigos2.Length);
                Instantiate (enemigos2[rand],transform.position,Quaternion.identity);
                    if (tiempo_inicio > tiempo_minimo) {
                        tiempo_inicio -= decremento;
                     }
                    tiempo_entre_generacion = tiempo_inicio;
            
            } else {
                tiempo_entre_generacion -= Time.deltaTime;
            }

        } else {
            cuenta_atras -= Time.deltaTime;
        }
    }
}
